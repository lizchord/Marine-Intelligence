...
from core.cleanlab.typing import LabelLike
from core.cleanlab.internal.multilabel_utils import stack_complement, get_onehot_num_classes
from core.cleanlab.internal.constants import TINY_VALUE, CONFIDENT_THRESHOLDS_LOWER_BOUND

from core.cleanlab.internal.util import (
...
from core.cleanlab.internal.latent_algebra import (
    compute_inv_noise_matrix,
    compute_py,
    compute_noise_matrix_from_inverse,
)
from core.cleanlab.internal.validation import (
    assert_valid_inputs,
    labels_to_array,
)
...
def num_label_issues(
    ...
    *,
    ...
    if estimation_method == "off_diagonal":
        _, cl_error_indices = compute_confident_joint(labels=labels, pred_probs=pred_probs, calibrate=False,
                                                      return_indices_of_off_diagonals=True)
        print(cl_error_indices)
        ...
        calculated_confident_joint = compute_confident_joint(labels=labels, pred_probs=pred_probs, calibrate=True)
...
def estimate_joint(
    ...
    * Satisfy ``np.sum(joint_estimate, axis = 1) == p(labels)``
    ...
    if confident_joint is None:
        calibrated_cj, indices = compute_confident_joint(labels, pred_probs, calibrate=True, multi_label=multi_label)
    else:
    ...
    if multi_label:
        ...
        return calibrated_cj / np.clip(float(np.sum(calibrated_cj)), a_min=TINY_VALUE, a_max=None), indices
...
def _estimate_joint_multilabel(
    ...
    if confident_joint is None:
        calibrated_cj, indices = compute_confident_joint(labels, pred_probs, calibrate=True, multi_label=True)
    else:
        calibrated_cj = confident_joint
    assert isinstance(calibrated_cj, np.ndarray)
...
def compute_confident_joint(
    ...
    *,
    ...
    calibrate: bool = True,
    multi_label: bool = False,
    return_indices_of_off_diagonals: bool = True,
...
def _compute_confident_joint_multi_label(
    ...
    *,
    ...
    for class_num, (label, pred_prob_for_class) in enumerate(zip(y_one.T, pred_probs.T)):
        pred_probs_binary = stack_complement(pred_prob_for_class)
        if return_indices_of_off_diagonals:
            cj, ind = compute_confident_joint(labels=label, pred_probs=pred_probs_binary, thresholds=thresholds,
                                              calibrate=calibrate, multi_label=False,
                                              return_indices_of_off_diagonals=return_indices_of_off_diagonals)
            indices_off_diagonal.append(ind)
        else:
            cj = compute_confident_joint(labels=label, pred_probs=pred_probs_binary, thresholds=thresholds,
                                         calibrate=calibrate, multi_label=False,
                                         return_indices_of_off_diagonals=return_indices_of_off_diagonals)
...
def estimate_py_and_noise_matrices_from_probabilities(
    ...
    *,
    ...
    confident_joint = compute_confident_joint(labels=labels, pred_probs=pred_probs, thresholds=thresholds,
                                              calibrate=calibrate)
